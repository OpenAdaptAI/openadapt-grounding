"""Deployment module for UI-TARS on AWS EC2 via vLLM.

UI-TARS 1.5-7B is ByteDance's SOTA UI grounding model. This module deploys it
        ...
        if e.response["Error"]["Code"] == "InvalidGroup.NotFound":
            try:
                response = ec2.create_security_group(
                    ...
                    Description="Security group for UI-TARS deployment",
                    TagSpecifications=[
                        {
...
class Deploy:
    """UI-TARS deployment manager."""

    @staticmethod
    def start() -> str:
        """Deploy UI-TARS and return server URL.

        Returns:
            Server URL (e.g., "http://1.2.3.4:8001")
        """
        try:
            ...
            if dockerfile_path.exists():
                print(f"Copying {dockerfile_path.name}...")
                subprocess.run(
                    [
                        "scp",
                        ...
                        str(dockerfile_path),
                        f"{config.AWS_EC2_USER}@{instance_ip}:~/Dockerfile",
                    ],
                    check=True,
                )
            ...
            try:
                ...
                # Wait for server (vLLM takes longer to load model)
                print("Waiting for vLLM server to load model (this may take 2-5 minutes)...")
                time.sleep(30)

                max_retries = 60  # 10 minutes total
                for attempt in range(max_retries):
                    try:
                        execute_command(ssh_client, f"curl -s http://localhost:{config.PORT}/health")
                        break
                    except Exception:
    ...
    def run() -> None:
        ...
        output = Deploy._run_ssh_command(
            ...
            f"-p {config.PORT}:8000 {config.PROJECT_NAME}:latest"
        )
        if output:
    ...
    def test(save_output: bool = False) -> None:
        """Test UI-TARS grounding endpoint with a synthetic image.
...
Click on the Login button"""
            ...
            print("\nSending grounding request: 'Click on the Login button'...")
            ...
            import re
            point_match = re.search(r"<point>(\d+)\s+(\d+)</point>", result)
            if point_match:
                x, y = int(point_match.group(1)), int(point_match.group(2))
                print(f"\nExtracted coordinates: ({x}, {y})")

                # Check if coordinates are in Login button area
                # Button is at [30, 30, 150, 70]
            ...
            print("Install with: uv pip install openai pillow")
        except Exception as e:
